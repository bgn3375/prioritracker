...
export function useData(user) {
  ...
  async function addTask(priorityId, data) {
    const { data: row, error } = await supabase.from('tasks').insert({
      ...
      status: data.status || 'todo', phase: data.phase || 'Planificare Analiză',
  ...
  async function importPriority(jsonData) {
    const d = jsonData;
    const np = await addPriority({ modul: d.priority.modul || 'Core — Conta', name: d.priority.name,
      status: d.priority.status || 'Analiză', description: d.priority.description || '',
    ...
    const tis = (d.tasks || []).map((t, i) => ({ priority_id: np.id, name: t.name, owner: t.owner,
      hours: t.hours || 1, status: 'todo', phase: t.phase || 'Planificare Analiză',
