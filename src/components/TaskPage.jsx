  ...
  return (
    <SidePanel onClose={onClose} zIndex={1050}>
      <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
        <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 14, color: "#94A3B8" }}>← Înapoi</button>
        <div style={{ flex: 1 }} />
        <button onClick={onEdit} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 14, color: "#94A3B8" }}>✎ Editează</button>
      </div>
      ...
      <button onClick={onToggle} style={{ width: "100%", padding: 10, borderRadius: 10, border: "none", cursor: "pointer", background: ds === "done" ? "#F0FDF4" : GRAD, color: ds === "done" ? "#15803D" : "#fff", fontSize: 13, fontWeight: 600, marginBottom: 20 }}>
        {ds === "done" ? "✓ Done — click pentru a anula" : "Marchează Done"}
      </button>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
          <input value={link} onChange={e => setLink(e.target.value)} onKeyDown={e => e.key === "Enter" && handleAddLink()} placeholder="Adaugă link..." style={{ flex: 1, ...iS, padding: "6px 10px" }} />
